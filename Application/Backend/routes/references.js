...
router.post('/private/createReference', async function(req, res) {
    if(!(req.body.Title && req.body.Authors && req.body.Publication && req.body.Description && req.body.Link)){
        res.status(400).send("Cannot create a Context without a Title, Authors, Publicatoin, Description adn Link fields")
    }else{
        try {
